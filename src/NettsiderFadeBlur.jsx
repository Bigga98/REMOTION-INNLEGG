...
export const NettsiderFadeBlur = () => {
  ...
  // Card animations — staggered blur/fade
  const cardAnims = grid.map((card, i) => {
    const delays = [16, 20, 24, 28];
    ...
    const blur = interpolate(prog, [0, 1], [12, 0]);
    const opacity = interpolate(prog, [0, 1], [0, 1]);

    return { x, y: finalY, blur, opacity, rotate: card.rotate };
  });
  ...
  return (
    <AbsoluteFill>
      ...
      >
        ...
        >
          <defs>
            <radialGradient id="leafFadeBlur" cx="40%" cy="40%" r="60%">
