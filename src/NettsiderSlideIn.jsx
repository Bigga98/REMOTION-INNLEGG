...
export const NettsiderSlideIn = () => {
  ...
  // Card animations — staggered by row, slide in from sides
  const cardAnims = grid.map((card, i) => {
    const delay = card.row === 0 ? 16 : 30;
    const prog = spring({
      frame: frame - delay,
      fps,
    ...
    const finalX = GRID_LEFT + card.col * (CARD_W + GAP);
    const startX = card.col === 0 ? -400 : 1080 + 200;
    const x = interpolate(prog, [0, 1], [startX, finalX]);

    const yOffset = card.side === "right" ? -12 : 0;
    const y = card.row * (CARD_H + GAP) + yOffset;

    const opacity = interpolate(prog, [0, 0.3], [0, 1], {
      extrapolateRight: "clamp",
    });

    return { rotate: card.rotate, y, opacity, x };
  });
  ...
  return (
    <AbsoluteFill>
      ...
      >
        ...
        >
          <defs>
            <radialGradient id="leafSlideIn" cx="40%" cy="40%" r="60%">
